...
- 2026-07-03 Draft/Review round 1: 初版把 `searchhistline(scr)` 误接到 `ST_ZIG_TERM_LINE_READ_FLAT_HISTORY_Y`，`code-reviewer` 指出这是坐标系错误：`searchhistline` 的 `scr` 是 ring offset，不是平铺历史行号。该轮 review 拒绝直接替换方案。
- 2026-07-03 Draft/Review round 2: 新增 `ST_ZIG_TERM_LINE_READ_RING_OFFSET` / `.ring_offset`，用 `historyIndex(histi, scr, histsize)` 表达 ring offset 语义；`searchhistline()` 改用 `RING_OFFSET`。补充 Zig 单测覆盖 `ring_offset` 与 export round-trip。
...
- 2026-07-03 Regression fix: `tlinehist()` 不能采用 `RING_OFFSET`，因为 `externalpipe()` 传入的是平铺历史/屏幕行号，首行必须映射到 `historyLine()` 的 hist index 0；`RING_OFFSET` 只适用于 `searchhistline(scr)` 的 ring offset 坐标。修复为 `ST_ZIG_TERM_LINE_READ_FLAT_HISTORY_Y`，并新增 external pipe row order 回归测试。

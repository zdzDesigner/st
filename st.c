...
#define TLINE(y)                                                               \
  ((y) < term.scr                                                              \
       ? term.hist[(                                                           \
             int)((((y) + term.histi - term.scr + HISTSIZE + 1) % HISTSIZE))]  \
...
void selnormalize(void) {
  ...
  result =
      st_selnormalizeupdate(selectionsnapshot(), term.col, start_len, end_len);
...
int searchmatch(int x, int y) {
  ...
  return st_searchmatchlist((const ZigSearchMatch *)search.matches,
                            matches.count, state.active, state.current,
                            term.scr, x, y);
}
...
static void searchwriteinputstate(SearchInputState state) {
...
static void searchapplyupdate(ZigSearchStateUpdate update) {
...
static void searchapplyinputinsert(ZigSearchInputResult result,
  ...
  memmove(search.input + result.move_dst, search.input + result.move_src,
          result.move_len);
...
static void searchapplycursor(int action) {
  ...
  result = st_searchcursorupdate(
      searchsnapshot(), (const unsigned char *)searchinputtext(), action);
...
size_t searchnextchar(size_t cursor) {
  ...
  while (next < input.len &&
         (((unsigned char)search.input[next] & 0xc0) == 0x80))
    next++;
  return next;
}

void searchdelete(size_t start, size_t end) {
...
void searchscanline(Line line, int scr, int y) {
  ...
  for (;;) {
    ...
    step = st_searchscanlineiter((const ZigGlyph *)line, term.col, search.query,
                                 state.query_len, scanstate);
    ...
    if (step.kind == ST_ZIG_SEARCH_APPEND_GROW) {
      ...
      searchwritematchesstate(matches);
      search.matches =
          xrealloc(search.matches, matches.cap * sizeof(*search.matches));
    }
...
void searchjump(void) {
  ...
  plan = st_searchjumpplan(state.active, state.current, matches.count, term.scr,
                           match->scr);
...
void selsnap(int *x, int *y, int direction) {
  ...
  case SNAP_WORD:
    ...
    for (;;) {
      word_request = st_selsnapworditerrequest(*x, *y, direction, term.col,
                                               term.row, prevdelim, prevrune);
      if (word_request.action == ST_ZIG_SEL_SNAP_WORD_ITER_STOP)
        break;

      wrap_allowed =
          !word_request.wrapped ||
      ...
      word_step = st_selsnapworditerresolve(word_request, prevdelim, prevrune,
                                            word_reader);
  ...
  case SNAP_LINE:
    ...
    *x = st_selsnaplinex(direction, term.col);
    if (direction < 0) {
      for (;;) {
        line_step = st_selsnaplinestep(
            *y, direction, term.row,
...
int tattrset(int attr) {
  return st_tattrset((const ZigGlyph *const *)term.line, term.row, term.col,
                     attr);
}
...
void treset(void) {
  ...
  term.c = (TCursor){{.mode = plan.cursor_attr_mode,
                      .fg = plan.cursor_fg,
                      .bg = plan.cursor_bg},
                     .x = plan.cursor_x,
                     .y = plan.cursor_y,
...
void tscrolldown(int orig, int n, int copyhist) {
  ...
  plan = st_tscrollplan(n, orig, term.bot, term.scr, HISTSIZE, 0, copyhist,
                        term.histi);
  n = plan.count;
...
void tscrollup(int orig, int n, int copyhist) {
  ...
  plan = st_tscrollplan(n, orig, term.bot, term.scr, HISTSIZE, 1, copyhist,
                        term.histi);
  n = plan.count;
...
void tsetattr(int *attr, int l) {
  ...
  case ST_ZIG_COLOR_BAD_RGB:
    fprintf(stderr, "erresc: bad rgb color (%u,%u,%u)\n", update.color_error.r,
            update.color_error.g, update.color_error.b);
    break;
...
void tapplyerase(const ZigErasePlan *plan) {
  ...
  for (i = 0; i < plan->count; ++i) {
    tclearregion(plan->rects[i].x1, plan->rects[i].y1, plan->rects[i].x2,
                 plan->rects[i].y2);
  }
}
...
void csihandle(void) {
  ...
  plan = st_csiexecplan(csiescseq.mode[0], csiescseq.mode[1], csiescseq.priv,
                        csiescseq.arg, csiescseq.narg, term.c.x, term.c.y,
                        term.col, term.row);
  ...
  case ST_ZIG_CSI_EXEC_MISC:
    if (plan.misc.kind == ST_ZIG_MISC_SET_CURSOR_STYLE &&
        xsetcursor(plan.misc.value)) {
      fprintf(stderr, "erresc: unknown csi ");
      csidump();
      break;
    }
...
void tcontrolcode(uchar ascii) {
  ZigInputControlPlan plan =
      st_inputcontrolplan(ascii, term.esc, term.charset, term.c.x);
...
int eschandle(uchar ascii) {
  ZigInputEscPlan exec =
      st_inputescplan(ascii, term.esc, term.charset, term.icharset, term.c.x);
  ZigNewlinePlan plan;
...
void tputc(Rune u) {
   ...
   */
  if (term.esc & ESC_STR) {
    collect_exec = st_tcollectstr(u, term.esc, (unsigned char *)strescseq.buf,
                                  strescseq.len, (const unsigned char *)c, len,
                                  strescseq.siz);
    ...
    if (collect_exec.kind == ST_ZIG_STR_COLLECT_GROW) {
      ...
      collect_exec = st_tcollectstr(u, term.esc, (unsigned char *)strescseq.buf,
                                    strescseq.len, (const unsigned char *)c,
                                    len, strescseq.siz);
    }
...
check_control_code:
  ...
  if (control) {
    ...
    escflow =
        st_inputescflowplan(term.esc, u, csiescseq.len, sizeof(csiescseq.buf));
    esc_action_done = 1;
    switch (escflow.kind) {
    ...
    if ((escflow.kind == ST_ZIG_ESC_FLOW_CSI ||
         escflow.kind == ST_ZIG_ESC_FLOW_ESC) &&
        !esc_action_done)
      return;
    term.esc = 0;
    /*
  ...
  prepare = st_tputcprepare(selected(term.c.x, term.c.y), IS_SET(MODE_WRAP),
                            term.c.state, term.c.x, width, term.col);
...
void draw(void) {
  ...
  if (!xstartdraw())
    return;
  frame = st_drawexecplan(
      search.active, term.scr, cursor_x, term.c.y, term.ocx, term.ocy, term.col,
      term.row, (const ZigGlyph *const *)term.line, term.dirty, 0, term.row);
  ...
  if (frame.cursor_active)
    xdrawcursor(cursor_x, term.c.y, term.line[term.c.y][cursor_x], term.ocx,
                term.ocy, term.line[term.ocy][term.ocx], term.line[term.ocy],
                term.col);
  term.ocx = cursor_x;
  term.ocy = term.c.y;
  xfinishdraw();

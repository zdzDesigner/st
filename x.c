...
{
    if ((alpha > 0 && arg->f < 0) || (alpha < 1 && arg->f > 0)) alpha += arg->f;
...
void selrequest(XEvent *e)
{
    ...
    if (!XSendEvent(xsre->display, xsre->requestor, 1, 0, (XEvent *)&xev)) fprintf(stderr, "Error sending SelectionNotify event\n");
}
 ...
 */
void xclear(int x1, int y1, int x2, int y2) { XftDrawRect(xw.draw, &dc.col[IS_SET(MODE_REVERSE) ? defaultfg : defaultbg], x1, y1, x2 - x1, y2 - y1); }
...
void xloadsparefonts(void)
{
    ...
    for (fp = font2; fp - font2 < fc; ++fp) {
        ...
        if (xloadsparefont(pattern, FRC_NORMAL)) die("can't open spare font %s\n", *fp);

        FcPatternDel(pattern, FC_SLANT);
        FcPatternAddInteger(pattern, FC_SLANT, FC_SLANT_ITALIC);
        if (xloadsparefont(pattern, FRC_ITALIC)) die("can't open spare font %s\n", *fp);

        FcPatternDel(pattern, FC_WEIGHT);
        FcPatternAddInteger(pattern, FC_WEIGHT, FC_WEIGHT_BOLD);
        if (xloadsparefont(pattern, FRC_ITALICBOLD)) die("can't open spare font %s\n", *fp);

        FcPatternDel(pattern, FC_SLANT);
        FcPatternAddInteger(pattern, FC_SLANT, FC_SLANT_ROMAN);
        if (xloadsparefont(pattern, FRC_BOLD)) die("can't open spare font %s\n", *fp);
...
void xinit(int cols, int rows)
{
    ...
    xw.win = XCreateWindow(xw.dpy, parent, xw.l, xw.t, win.w, win.h, 0, xw.depth, InputOutput, xw.vis, CWBackPixel | CWBorderPixel | CWBitGravity | CWEventMask | CWColormap,
                           &xw.attrs);
...
int xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len, int x, int y)
{
    ...
    for (i = 0, xp = winx, yp = winy + font->ascent; i < len; ++i) {
        ...
        if (f >= frclen) {
            ...
            if (!frc[frclen].font) die("XftFontOpenPattern failed seeking fallback font: %s\n", strerror(errno));
...
int resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst)
{
    ...
    if (ret.addr == NULL || strncmp("String", type, 64)) return 1;
...
void config_init(void)
{
    ...
    for (p = resources; p < resources + LEN(resources); p++) resource_load(db, p->name, p->type, p->dst);
}

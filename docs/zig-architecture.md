...
- Resize、draw 和 graphic putc 路径在设计上都曾尝试进一步收口到 ordered step / effect list / state-update seam，但 2026-07-02 的 copy/delete 显示异常复盘证明：terminal 显示链路对中间状态时序极度敏感，不能把“字段上等价”直接当成“行为上等价”。当前稳定实现仍复用 `ZigResizeExecPlan`、`ZigDrawExecPlan`、`st_drawregionplan(...)` 和 `ZigTermCursorSnapshot -> st_termcursorplan` 提供范围与查询能力，但 `st.c` 已把 `tresize()`、`draw()` 和 `tputc()` 图形写入尾段分别收敛回更接近 `8ea88fe7` 的旧消费语义，尤其是 dirty 与 `CURSOR_WRAPNEXT` 必须在单次 glyph 写入后同拍提交。详见 `docs/reports/20260702-copy-delete-root-cause.md`。
- 后续拆分 `st.c` / `x.c` 前必须先查 `docs/terminal-effect-ordering.md`：该文档把必须原子提交、必须先后执行、纯决策和只能留在 C executor 的边界列为迁移护栏。
...
- TermState 标量写回仍以 `ZigTermStateUpdate -> termapplystateupdate()` 作为主要 seam；`tsetmode()` 的 `CURSOR_ORIGIN`、reset 的 cursor/scroll region/mode/charset/trantbl 写回仍由 Zig update 返回。但 `tputc()` graphic advance 的 `CURSOR_WRAPNEXT` 已不再视为安全的独立 state-update 字段消费点：2026-07-02 的显示异常修复证明，这个位必须与 glyph 写入结果在同一个提交时序里落地，不能仅依赖 planner 预先返回的 `cursor_state_mask/cursor_state_bits`。详见 `docs/reports/20260702-copy-delete-root-cause.md`。
...
- Input 已完成首批聚合：`tcontrolcode`、`eschandle` 和 `tputc` ESC flow 仍改用 `st_inputcontrolplan`、`st_inputescplan`、`st_inputescflowplan`；`ZigInputControlPlan` / `ZigInputEscPlan` 仍是稳定的 action/ret/finish + state payload seam，`inputapplycontrolplan()` 也仍是共享 executor；backspace 仍通过 `st_backspaceplan()` 保持与正常分支一致，不跨 soft-wrap 行；`tputc` 顶层 route / print gate 仍由 `ZigInputStepPlan` 返回，覆盖 STR/control/ESC/graphic 路径；但 graphic 路径的“写 glyph 后 dirty + `CURSOR_WRAPNEXT` 如何提交”已被 2026-07-02 的显示异常修复重新收紧：`ZigPutcStepPlan` 当前只适合作为 selection clear 与 wrap/overflow newline 决策来源，不应被视为完整替代 `st_tputcwrite(...)` 写入结果消费语义的稳定边界。详见 `docs/reports/20260702-copy-delete-root-cause.md`。

...
- `st_mode.zig` 通过 `ModeParam`、`Utf8Selector`、`CharsetSelector`、`AltScreen` 承载 mode、UTF-8、charset、alternate screen swap、mouse mode 和 cursor save/load 参数分类；`1049/47/1047/1048` 的 cursor/clear/swap action 顺序与 mouse mode 的 pointer/clear/set action 已由 Zig plan 承载，C executor 只执行 `tcursor`、`tclearregion`、`tswapscreen`、`xsetpointermotion`、`xsetmode` 等副作用。
...
- Mode actions 已完成前两条低风险切片：alternate screen / cursor save-load 路径不再依赖 C fallthrough 表达顺序，mouse mode 路径不再由 C 重复维护 pointer/clear/set 顺序；`ZigModePlan` 返回 action fields，C 继续执行真实副作用和 `allowaltscreen` gate。

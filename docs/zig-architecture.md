...
- `st_cursor.zig` 通过 `TermCursor`、`ZigTermCursorSnapshot`、`ZigTermCursorPlan`、`BackspacePlan`、`DrawCursor` 和 `DrawExecPlan` 承载移动 clamp、origin-mode y 调整、换行、reverse-index、soft-wrap backspace、保存/恢复动作、draw frame/region 执行规划、draw platform effect ordering 和 IME spot 更新判断。

...
以下用例用于验证 `tcommitputcwrite` 一类提交边界没有再次把 dirty 与 `CURSOR_WRAPNEXT` 拆散：

1. 行尾窄字符写入：光标置最后一列，输入 `A`，只允许当前行末尾出现 `A`。
2. 行尾窄字符后继续输入：场景 1 后输入 `B`，应换行到下一行开头，不得覆盖当前行内容。
3. 行尾宽字符写入：光标置倒数第二列，输入 `中`，应正常占两格或按预期换行，不得留单格残影。
4. copy-then-delete：选中文本后粘贴到行尾，立即 delete 删除；删除后行内容应恢复，不得下一帧再次长出已删文本。
5. 持续光标移动增殖：场景 4 后仅左右移动光标，内容必须保持不变，不能每移动一次多复制一份。
6. 行尾 + INSERT mode：在 INSERT mode 的行尾输入字符，不得把现有内容错写到其他位置。
7. wrapnext 连续写入：光标已在行尾并带 `CURSOR_WRAPNEXT` 时输入 `X`，`X` 应落到下一行开头，当前行不再改动。
8. overflow 连续写入：光标在行末且无 wrapnext 时输入超出列数的字符，应走 overflow 换行路径，不得写回上一行。

...
- clipboard paste 仍由 `x.c` 直接接收 X selection 并写入 PTY。`CLIPBOARD` paste 先请求 `TARGETS`：文本优先走 `UTF8_STRING` / `XA_STRING` 且 `format == 8` 的文本 payload；没有文本但有 `image/png` 时保存为 `/tmp/st-clipboard-image-*.png`，再向 PTY 粘贴裸路径，供 opencode 的 paste handler 按本地图片路径转成 `[Image N]` 附件；其他非文本 selection 在 C 边界丢弃。

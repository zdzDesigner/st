...
- **Mode/State 标量收口**：`tsetmode` 的 WRAP/INSERT/ECHO/CRLF 位写入由 `ZigModePlan.mode_mask/mode_bits` 驱动，`tdefutf8` / `tdeftran` 复用 `st_mode.zig` 的 selector 逻辑，media print mode 位写入由 `ZigMiscPlan.mode_mask/mode_bits` 驱动；C 侧只执行最终 `term.mode` / charset 写回和平台 mode 副作用。

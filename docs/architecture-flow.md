...
- **Resize 收口**：`ZigResizeExecPlan` 仍持有 resize 范围数据，但 2026-07-02 的 copy/delete 显示异常复盘表明，`st.c` 的 `tresize()` 执行顺序不能仅凭 ordered step list 视为已行为定版；当前稳定实现重新贴回 `8ea88fe7` 的旧顺序语义，继续复用 Zig 返回的 range 数据，但不再直接依赖 step executor 作为唯一真实执行顺序。详见 `docs/reports/20260702-copy-delete-root-cause.md`。
- **Draw/TermState 大步收口**：draw frame gate、cursor 调整和 dirty region 查询仍由 `ZigTermFrameSnapshot`、`ZigDrawExecPlan` 和 `st_drawregionplan(...)` 提供数据与查询能力；但 2026-07-02 的 copy/delete 显示异常复盘证明，draw/dirty/cursor 这类显示路径不能只按“字段等价”判断收口完成。当前稳定实现保留 `searchscan -> drawregion -> xdrawcursor -> xfinishdraw` 的旧主流程消费语义，`drawregion()` 重新显式控制 dirty scan loop；文档里所有“draw platform effects 已行为定版”的判断，都必须以真实交互验证为准。详见 `docs/reports/20260702-copy-delete-root-cause.md`。
...
- **STR/Input collection 收口**：`ZigInputStepPlan` 仍负责 `tputc` 的 STR/control/ESC/graphic 顶层路由和 print gate，`ZigInputControlPlan` / `ZigInputEscPlan` 也仍是稳定边界；但 2026-07-02 的 copy/delete 显示异常复盘证明，graphic 路径不能把“写 glyph 后的 dirty/cursor-state 提交”机械拆成 `ZigPutcStepPlan` 的独立字段再逐项消费。当前稳定实现只复用 `ZigPutcStepPlan` 的 selection clear 与 wrap/overflow newline 决策，图形字符写入后的 dirty 与 `CURSOR_WRAPNEXT` 提交重新回到旧式 `st_tputcwrite(...)` 返回值语义。详见 `docs/reports/20260702-copy-delete-root-cause.md`。

...
Platform shim 的实现入口当前只开放 effect-ordering plan，不迁移 X11 display、PTY fd、clipboard selection 或 IO fd ownership；真实平台资源 ownership 必须等前三个 owner project 稳定后再评估。第一版 `PlatformEffectList -> platformapplyeffects()` 已承接 STR/title/clipboard/color effect ordering。
...
- **STR/Input collection 收口**：`st_inputroutingplan()` 返回 `tputc` 的 STR/control/ESC/graphic 路由；graphic 路径通过 `ZigPutcStepPlan` 合并 selection clear、wrap/overflow newline、glyph write、dirty mark 和 cursor advance 规则；`st_tcollectstrapply()` 返回 STR append/grow 后是否 retry，`st_strresetplan()` 返回由 `ST_ZIG_STR_BUF_SIZ` 同步的初始 buffer size，`st_strparse()` 返回参数 start/end 和 NUL 分隔写入元数据，`st_strhandleparplan()` / `st_strhandleplan()` 返回主参数解析规则和 payload/color 参数索引；`st_strapplyplan()` 返回 title/clipboard/color 有序 effect list。C 侧通过 `strapplyplan()` 顺序执行真实 X11/clipboard/color 副作用，仍执行 `xrealloc`、持有 `strescseq.buf` 指针并按 plan 原地写入 NUL 分隔符。
...
- **TermState 写回收口**：mode mask/bits、cursor state mask/bits、charset、translation table、scroll region 和 resize base 写回已统一进入 C-side `TermStateUpdate -> termapplystateupdate()`；`CURSOR_ORIGIN` 和 `CURSOR_WRAPNEXT` 的业务写回已由 Zig plan 返回，reset 仍保留 `ZigResetPlan` adapter，但最终也通过同一 apply seam 写回 cursor、scroll region、mode、charset 和 translation table。
